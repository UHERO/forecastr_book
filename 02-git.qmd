...
While in the dev branch, make changes to scripts or other files and save them. When you are ready to *commit* the changes, *stage* the files in the Git pane ("Command a" selects all files), click "Commit" at the top of the Git pane, enter a commit message, and click the "Commit" button. Then *push* the changes to the remote repository. Go to GitHub and create a *pull request* to *merge* the dev branch into the main branch. After the pull request is merged, delete the dev branch on GitHub. Locally you can delete the dev branch by clicking on the "More" button in the Git pane and selecting "Delete branch".

This workflow is recommended to avoid conflicts with other collaborators.


If the push is rejected, pull the changes from the remote repository by clicking the "Pull" button in the Git pane. If there are conflicts, resolve them by editing the files and committing the changes. Push the changes to the remote repository by clicking the "Push" button in the Git pane. If you are working in a branch, push the branch to the remote repository by clicking the "Push" button in the Git pane.



Pull changes from the remote repository by clicking on the "Pull" button in the Git pane. If there are conflicts, resolve them by editing the files and committing the changes. Push the changes to the remote repository by clicking on the "Push" button in the Git pane. If the push is rejected, pull the changes again, resolve any conflicts, and push the changes again. If you are working in a branch, push the branch to the remote repository by clicking on the "Push" button in the Git pane. 
Commit changes to the branch, push the branch to the remote repository, and create a pull request on GitHub to merge the branch into the main branch. After the pull request is merged, delete the branch on GitHub. This workflow is recommended to avoid conflicts with other collaborators.
